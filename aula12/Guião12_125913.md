...
- O resultado está de acordo com o previsto.

### k)

![1k)](screenshots/1k.png)

# Exercício 2. *POPEN*

### a)

- popen() system call que abre uma pipe para um processo em execução.
- O programa popen1.c começa por definir uma FILE *fp e um buffer de 1024 caracteres, de segida abre uma pipe com popen para executar o comando ls -l no modo leitura *"r"* (vamos ler a saída do comando ls -l) e retorna um ponteiro para um fluxo de ficheiro guardado em fp. Em seguida verifica se o conteúdo foi corretamente carregado pra fp.
- De seguida, o programa começa um loop while que diz que enquanto houver linha no fscanf de fp com o buffer atribuido, faz print da linha com o prefixo "frompipe:".
- O output esperado será de uma lista com os ficheiros e diretorios, e respetivas permissões, de todos os ficheiros no caminho atual junto com o prefixo "frompipe:".

### b)

![2b)](screenshots/2b.png)

# Exercício 3. *Named pipes*

### a)

- makefifo() é uma systemcall que cria named pipes (FIFOs) with the given names.
- O programa *namedpipe1.c* começa por criar um FIFO com o nome especificado (myfifo1) e as suas permissẽs (0600), se ocurrer um erro na criação (se o retorno for diferente de 0) dá-se perror().

### b)

- Como o código correu como esperado, não é escrito nada no terminal, ao correr o comando "ls -l myfifo1" no terminal, conseguimos ver o FIFO:
  ls -l myfifo1
  prw------- 1 tiago tiago 0 Dec 12 00:13 myfifo1
  p -> significa que é um fifo

### c)

- O código começa por declarar uma FILE *fp e um buffer de 1024 caracteres. De seguida tenta abrir o FIFO chamado myfifo1 para escrita.
- De seguida o programa imprime ">" e enquanto for for lido mensagens do terminal, são armazenadas no buffer e escreve no FIFO a mesma mensagem. 

### d)

- O motivo para esta anomalia, é porque os FIFOs são usados para comunicação entre processos, ou seja, como há apenas 1 processo a tentar escrever no fifo, este será bloqueado até que outro processo leia do FIFO. Concluido, o simbolo ">" não aparece porque o programa está a agurdar a leitura do FIFO.

### e)

- Agora já aparece o simbolo porque há 2 processos a trabalhar com o FIFO (a ler e a escrever) logo o FIFO vai desbloquear. À medida que mandamos mensagens no terminal do writepipe1 aparece no terminal do cat as mensagens que escrevemos

### f)

![3f)](screenshots/3f.png)

### g)

![3g)](screenshots/3g.png)